defmodule ProjectApiCreateTest do
  ...
  test "creates a new project" do
    ...
    assert %{"data" => %{"machine_key_id" => machine_id, "project_id" => project_id}} = Jason.decode!(conn.resp_body)

    assert {:ok, _resp} = Project.can_access(%{auth_header: @is_authorized_header, request_id: "234", token: token, id: project_id})
    assert {:ok, %Response{data: %{"key" => encrypted_machine_key, "project_id" => ^project_id}}} = Secret.get_machine_key(%{auth_header: @get_machine_key_header, request_id: "345", id: machine_id})
